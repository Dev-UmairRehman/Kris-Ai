...
const config = {
  ...
  buddypro: {
    ...
    /* MEASURED, not assumed - see `npm run probe:language`.
       x_buddy_systemPrompt is ACCEPTED (HTTP 200) but has NO EFFECT on this
       instance: the profile still opened with its "rules of engagement"
       preamble even when this prompt explicitly forbade one. It is still sent
       because it costs nothing and may work on another instance, but it is NOT
       what fixes the language - `responseLanguage` below is. */
    systemPrompt: str(
      'BUDDYPRO_SYSTEM_PROMPT',
      'Always reply in English only, regardless of the language of the question. ' +
        'Never mix languages. Do not open with an introduction, a welcome, or any ' +
        'rules of engagement - answer the question directly in your first sentence.'
    ),
  },
  ...
  /* Controls audio UPLOAD only, and is off because this instance rejects it:
     audio-only requests return 500 "Error processing the message" for every
     format, sample rate and payload shape. Speech is therefore transcribed in
     the browser and sent as text.

     This does NOT gate spoken replies - BuddyPro's TTS does return audio (see
     lib/buddypro.js), and the call requests it on every turn. Flip this to true
     only once audio upload works; verify with `npm run probe:audio`. */
