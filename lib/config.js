...
const config = {
  ...
  buddypro: {
    ...
    /* Answer locally instead of calling BuddyPro. Every real request costs
       credits, so the test suites run against a mocked server by default -
       start it with BUDDYPRO_MOCK=true. Refused in production. */
    mock: str('BUDDYPRO_MOCK', 'false') === 'true',
...
if (config.isProd && config.buddypro.mock) {
  problems.push('BUDDYPRO_MOCK=true is refused in production - it fakes every answer.');
}
