...
const config = {
  ...
  /* In frame mode, require the store page to say that someone is signed in.
     A Uscreen landing page is PUBLIC, so being framed by strategytraining.com
     proves nothing on its own - without this, anyone reaching the page could
     use the widget and spend credits. Fails closed: no answer from the store
     snippet means no access. */
  requireStoreIdentity: str('REQUIRE_STORE_IDENTITY', 'true') === 'true',
