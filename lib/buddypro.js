...
async function ask(opts) {
  ...
  if (!hasAudio && !trimmed && !(Array.isArray(opts.images) && opts.images.length)) {
  ...
  /* Exactly one user message. Audio replaces everything when present; images
     ride alongside the text. */
  const images = Array.isArray(opts.images) ? opts.images : [];
  let content;

  if (hasAudio) {
    content = [{ type: 'input_audio', input_audio: { data: audioInput, format: audioFormat } }];
  } else if (images.length) {
    content = [
      {
        type: 'text',
        text: directive + (trimmed || 'Take a look at this and tell me what you make of it.'),
      },
    ].concat(
  ...
  /* Mocked: never touch the network, never spend a credit. Shaped like a real
     answer, including the blank-line splits the UI renders as separate
     bubbles. */
  if (config.buddypro.mock) {
    await new Promise((r) => setTimeout(r, 250));
    const heard = hasAudio ? 'I heard your voice message.' : trimmed.slice(0, 80);
    return {
      id: 'mock-' + Date.now(),
      content:
        'This is a mocked reply, so no credits were spent.' +
        '\n\nYou said: ' +
        (heard || '(nothing)') +
        '\n\nSet BUDDYPRO_MOCK=false to talk to the real Kris.',
      audio: null,
      image: null,
    };
  }
