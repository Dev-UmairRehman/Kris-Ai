...
async function ask(opts) {
  ...
  /* Spoken replies. NOTE: measured on this instance, modalities:['text','audio']
     returns HTTP 200 with NO audio object - BuddyPro's TTS is not coming back
     through the API even though the Telegram bot clearly has a voice. Left in
     place so it starts working the moment the instance enables it; the call
     currently speaks with the browser voice instead. */
  if (wantAudio) {
  ...
  if (config.buddypro.systemPrompt && config.buddypro.surface === 'owner') {
